...
<script>
import {cellphoneLogin,loginStatus,getUserInfo} from '@/plugins/axios.js'
export default {
    name: 'login',
    data(){
        return{
    ...
    async mounted(){
        function UserInfo(loginstatus,useravatarUrl,userlevel,followeds,follows,nickname,eventCount,userId,city) {
        ...
        let userid = resp1.bindings[0].userId;
        //登录成功 的话 将状态保存 到 登录模块，方便共享，此处本来想在登录模块加入getter属性，仔细一想想似乎没啥必要，
        //因为我没有什么需要共同计算的属性 我只是要这个状态进行渲染而已。
        if(resp1.code == 200){
            const{data: userinfo} = await getUserInfo({uid:userid});
            console.log(userinfo)
            let userId = userid;
            let useravatarUrl = userinfo.profile.avatarUrl
            let userlevel = userinfo.level
            let followeds = userinfo.profile.followeds
            let follows = userinfo.profile.follows
            let nickname = userinfo.profile.nickname
            let eventCount = userinfo.profile.eventCount
            let city = userinfo.profile.city
            if(city == 450700){
                city = "广西 - 钦州市"
            }else{
                city = "由于不知道地区的算法，所以你在哪我也不知道"
            }
            let userInfo = new UserInfo(true,useravatarUrl,userlevel,followeds,follows,nickname,eventCount,userId,city)
            this.$store.commit('moduleLogin/parseStateIndexUserInfo',userInfo)
